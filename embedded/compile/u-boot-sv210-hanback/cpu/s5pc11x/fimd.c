...
#if 1
  Outp32(0xf8000004, 0x01d3c060);
	Outp32(0xf8000010, 0x00060400);
	Outp32(0xf8000014, 0x000c0702);
	Outp32(0xf8000170, 0x00000000);
	Outp32(0xf8000018, 0x000efb1f);
	Outp32(0xf8000000, 0x0);
	Outp32(0xf8000000, 0x00000157);
	Outp32(0xf8000130, 0x00001001);
#else
	...
	Outp32(0xf8000020, 0x802c);
#else
	Outp32(0xf8000048, 0x0);
	Outp32(0xf8000200, 0x0);
	Outp32(0xf8000204, 0x0);
	Outp32(0xf8000034, 0x00000004);
	Outp32(0xf8000020, 0x00000000);
#endif
...
#elif defined(CFG_LCD_LTE480WV) || defined(CFG_LCD_LTP700WV)
#define LCD_WIDTH   800
#define LCD_HEIGHT  480

void LCD_Initialize_LTE480WV_LTP700WV(void)
{
	u32 uFbAddr = CFG_LCD_FBUFFER;
...
//#endif
  ...
  Outp32(0xf8000004, 0x01d3c060);
	...
	Outp32(0xf8000020, 0x802d);
	Outp32(0xf8000034, 0x1);
	Outp32(0xf8000000, 0x153);  /* HBE-SM3-SV210 */
	Outp32(0xf8000010, 0x60400);
	Outp32(0xf80001a4, 0x3);
...
#elif defined(CFG_LCD_LTE480WV) || defined(CFG_LCD_LTP700WV)
	LCD_Initialize_LTE480WV_LTP700WV();
#endif
}
...
{
#if defined(CFG_LCD_TL2796) || defined(CFG_LCD_NONAME1) || defined(CFG_LCD_LTE480WV) || defined(CFG_LCD_LTP700WV)
	u32 i, j;
...
#define BMP_LOGO_RAM_ADDR   0x24000000
#define BMP_LOGO_NAND_ADDR  0xFE00000
...
int LCD_ReadBmpLogo(void)
{
  ...
  size_t len;
  u_char *buf;
  ulong ram_addr, nand_off;

  len = BMP_LOGO_LENGTH;
  ram_addr = BMP_LOGO_RAM_ADDR;
  nand_off = BMP_LOGO_NAND_ADDR;
  nand_info_t *nand;
  nand = &nand_info[nand_curr_device];
  ret = nand_read(nand, nand_off, &len, (u_char *)ram_addr);
  return 0;
}

#define BMP_ROW 800
#define BMP_COL 480
void LCD_DrawBmpLogo(int dx_st,int dy_st,int dw_x, int dw_y)
{
  int t,tt, dw_offset;
  unsigned short pixel;
  unsigned char dw_r,dw_g,dw_b;
  unsigned short bmpdata2[BMP_ROW*BMP_COL];
  int i,k,j;
  unsigned char *dw_data;
  ...
  pfbdata = (unsigned short *)CFG_LCD_FBUFFER;

  dw_data = (unsigned char *)BMP_LOGO_RAM_ADDR;
  bmp_h = *(dw_data+10);
  dw_data = (unsigned char *)(BMP_LOGO_RAM_ADDR+bmp_h);  // Header Length ?

  for(j=0;j<dw_y;j++){
    k = j*dw_x*3;   // 0, 270, 540 ==> ++ 270 ea
    t = (dw_y -1 - j)*dw_x; // 4410, 4411
    for(i=0; i < dw_x; i++)
    {
      dw_b = *(dw_data + (k + i*3));    //b
      dw_g = *(dw_data + (k + i*3+1));  //g
      dw_r = *(dw_data + (k + i*3+2));  //r
      //pixel = ((dw_r >> 3) << 11) | ((dw_g >> 2) << 5) | ((dw_b >> 3) << 0);
      pixel = ((dw_r >> 3) << 16) | ((dw_g >> 2) << 8) | ((dw_b >> 3) << 0);
      bmpdata2[t+i] = pixel;
    }
  }
  for(t = 0; t < dw_y; t++)
  {
    dw_offset = (t + dx_st)* BMP_ROW;

    for(tt = 0; tt < dw_x; tt++)
    {
      pixel = bmpdata2[tt + t * dw_x];
      *(pfbdata + dw_offset + tt + dy_st) = pixel;
    }
  }
}
