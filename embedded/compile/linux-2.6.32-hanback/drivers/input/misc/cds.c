/*
 * CDS Device Driver
 *  Hanback Electronics Co.,ltd
 * File : cds.c
 * Date : April,2009
 */ 

// ¸ğµâÀÇ Çì´õÆÄÀÏ ¼±¾ğ
#include <linux/init.h>
#include <linux/module.h>
#include <linux/kernel.h>
#include <linux/fs.h>
#include <linux/errno.h>
#include <linux/types.h>
#include <linux/delay.h>
#include <asm/fcntl.h>
#include <linux/ioport.h>

#include <asm/ioctl.h>
#include <asm/uaccess.h>
#include <asm/io.h>

#include <mach/gpio.h>
#include <plat/gpio-cfg.h>
#include <mach/regs-gpio.h>

/* Prototypes */
extern int s3c_adc_get_adc_data(int channel);
#define ADC_CHANNEL         0 /* index for s5pC110 9â–~Râ–~R channel adc */

#define CDS_BASE            0xbd
#define IOCTL_CDS_READ      _IOR(CDS_BASE,0,int)

#define DRIVER_AUTHOR       "hanback"	// ¸ğµâÀÇ ÀúÀÛÀÚ
#define DRIVER_DESC		      "cds program" // ¸ğµâ¿¡ ´ëÇÑ ¼³¸í

#define	CDS_MAJOR_NUMBER	  301		// µğ¹ÙÀÌ½º ÁÖ¹øÈ£
#define	CDS_NAME		        "CDS"		// µğ¹ÙÀÌ½º ÀÌ¸§

//Global variable
static unsigned int   cds_usage = 0;
static unsigned long  cds_ioremap;

// define functions...
// ÀÀ¿ë ÇÁ·Î±×·¥¿¡¼­ µğ¹ÙÀÌ½º¸¦ Ã³À½ »ç¿ëÇÏ´Â °æ¿ì¸¦ Ã³¸®ÇÏ´Â ÇÔ¼ö
int cds_open (struct inode *inode, struct file *filp)
{
	// µğ¹ÙÀÌ½º°¡ ¿­·Á ÀÖ´ÂÁö È®ÀÎ.
	if(cds_usage != 0) return -EBUSY;

  /* Setting GPG2_3 => '1' */
  s3c_gpio_cfgpin(S5PV210_GPG2(3), S3C_GPIO_SFN(2));
  s3c_gpio_setpull(S5PV210_GPG2(3), S3C_GPIO_PULL_UP);

  cds_usage = 1;

	return 0;
}

// ÀÀ¿ë ÇÁ·Î±×·¥¿¡¼­ µğ¹ÙÀÌ½º¸¦ ´õÀÌ»ó »ç¿ëÇÏÁö ¾Ê¾Æ¼­ ´İ±â¸¦ ±¸ÇöÇÏ´Â ÇÔ¼ö
int cds_release (struct inode *inode, struct file *filp)
{
	cds_usage = 0;
	return 0;
}

// µğ¹ÙÀÌ½º µå¶óÀÌ¹öÀÇ ÀĞ±â¸¦ ±¸ÇöÇÏ´Â ÇÔ¼ö
ssize_t cds_read(struct file *inode, char *gdata, size_t length, loff_t *off_what)
{
	int ret, value;

  value = s3c_adc_get_adc_data(ADC_CHANNEL);

	// value°¡ °¡¸®Å°´Â Ä¿³Î ¸Ş¸ğ¸® µ¥ÀÌÅÍ¸¦ gdata°¡ °¡¸®Å°´Â »ç¿ëÀÚ
	// ¸Ş¸ğ¸® µ¥ÀÌÅÍ¿¡ n ¹ÙÀÌÆ® ¸¸Å­ ½á³Ö´Â´Ù.
	ret=copy_to_user(gdata,&value,4);

	if(ret<0) return -1;
	return length;
}

static int cds_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
  int value = 0;
  int size;

  size = _IOC_SIZE(cmd);

  switch (cmd)
  {
    case IOCTL_CDS_READ:
      value = s3c_adc_get_adc_data(ADC_CHANNEL);
      copy_to_user((void *)arg, (const void *)&value, (unsigned long) size);
//      mdelay(100);
//      printk("value=%d\n", value);
  }
  return 0;
}

// ÆÄÀÏ ¿ÀÆÛ·¹ÀÌ¼Ç ±¸Á¶Ã¼
// ÆÄÀÏÀ» ¿­¶§ open()À» »ç¿ëÇÑ´Ù. open()´Â ½Ã½ºÅÛ ÄİÀ» È£ÃâÇÏ¿© Ä¿³Î ³»ºÎ·Î µé¾î°£´Ù.
// ÇØ´ç ½Ã½ºÅÛ Äİ°ú °ü·ÃµÈ ÆÄÀÏ ¿¬»êÀÚ ±¸Á¶Ã¼ ³»ºÎÀÇ open¿¡ ÇØ´çÇÏ´Â ÇÊµå°¡ µå¶óÀÌ¹ö ³»¿¡¼­
// cds_open()À¸·Î Á¤ÀÇµÇ¾î ÀÖÀ¸¹Ç·Î cds_open()°¡ È£ÃâµÈ´Ù.
// write, read, releaseµµ ¸¶Âù°¡Áö·Î µ¿ÀÛÇÑ´Ù.
struct file_operations cds_fops = 
{
  .owner		= THIS_MODULE,
  .open		  = cds_open,
  .read		  = cds_read,
  .ioctl    = cds_ioctl,
  .release	= cds_release,
};

// ¸ğµâÀ» Ä¿³Î ³»ºÎ·Î »ğÀÔ
// ¸ğµâ ÇÁ·Î±×·¥ÀÇ ÇÙ½ÉÀûÀÎ ¸ñÀûÀº Ä¿³Î ³»ºÎ·Î µé¾î°¡¼­ ¼­ºñ½º¸¦ Á¦°ø¹Ş´Â °ÍÀÌ¹Ç·Î
// Ä¿³Î ³»ºÎ·Î µé¾î°¡´Â init()À» ¸ÕÀú ½ÃÀÛÇÑ´Ù.
// ÀÀ¿ë ÇÁ·Î±×·¥Àº ¼Ò½º ³»ºÎ¿¡¼­ Á¤ÀÇµÇÁö ¾ÊÀº ¸¹Àº ÇÔ¼ö¸¦ »ç¿ëÇÑ´Ù. ±×°ÍÀº ¿ÜºÎ
// ¶óÀÌºê·¯¸®°¡ ÄÄÆÄÀÏ °úÁ¤¿¡¼­ ¸µÅ©µÇ¾î »ç¿ëµÇ±â ¶§¹®ÀÌ´Ù. ¸ğµâ ÇÁ·Î±×·¥Àº Ä¿³Î
// ³»ºÎÇÏ°í¸¸ ¸µÅ©µÇ±â ¶§¹®¿¡ Ä¿³Î¿¡¼­ Á¤ÀÇÇÏ°í Çã¿ëÇÏ´Â ÇÔ¼ö¸¸À» »ç¿ëÇÒ ¼ö ÀÖ´Ù.
int cds_init(void)
{
	int result;

	// ¹®ÀÚ µğ¹ÙÀÌ½º µå¶óÀÌ¹ö¸¦ µî·ÏÇÑ´Ù.
	result = register_chrdev(CDS_MAJOR_NUMBER, CDS_NAME, &cds_fops);
	if (result < 0) {  // µî·Ï½ÇÆĞ
		printk(KERN_WARNING" %s HBE-SM3-SV210 CDS Device Driver initialized Faild\n",__FUNCTION__);
		return result;
	}
	
	// major ¹øÈ£¸¦ Ãâ·ÂÇÑ´Ù.
  printk("HBE-SM3-SV210 CDS Device Driver initialized Ver!! 1.0[%d] \n", CDS_MAJOR_NUMBER);
	return 0;
}

// ¸ğµâÀ» Ä¿³Î¿¡¼­ Á¦°Å	
void cds_exit(void)
{
	// ¹®ÀÚ µğ¹ÙÀÌ½º µå¶óÀÌ¹ö¸¦ Á¦°ÅÇÑ´Ù.
	unregister_chrdev(CDS_MAJOR_NUMBER,CDS_NAME);

	printk("driver: %s DRIVER EXIT\n", CDS_NAME);
}

module_init(cds_init);	// ¸ğµâ ÀûÀç ½Ã È£ÃâµÇ´Â ÇÔ¼ö
module_exit(cds_exit);	// ¸ğµâ Á¦°Å ½Ã È£ÃâµÇ´Â ÇÔ¼ö

MODULE_AUTHOR(DRIVER_AUTHOR);	// ¸ğµâÀÇ ÀúÀÛÀÚ
MODULE_DESCRIPTION(DRIVER_DESC); // ¸ğµâ¿¡ ´ëÇÑ ¼³¸í
MODULE_LICENSE("Dual BSD/GPL");	 // ¸ğµâÀÇ ¶óÀÌ¼±½º µî·Ï

