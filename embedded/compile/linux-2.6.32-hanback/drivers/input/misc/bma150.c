...
/* raoudi-20100930 */
static void bma150_config(struct i2c_client *client)
{

  unsigned char ret = 0;

  printk("###### %s ###### \n", __FUNCTION__);   // raoudi-20100930
	i2c_smbus_write_byte_data(client, 0x15, 0x0);   // Setting I2C Protocol
  ret = i2c_smbus_read_byte_data(client, 0xa);    //
  printk("###### REG_0xA = %x\n", ret);         
  i2c_smbus_write_byte_data(client,0xa, (ret | 0x10));
  i2c_smbus_write_byte_data(client, 0x35, 0x0);
  i2c_smbus_write_byte_data(client,0xa, (ret & (~0x10)));

  ret = i2c_smbus_read_byte_data(0x15, 0x0);
  if( ret & 0x80) printk("BMA150 Sensor SPI 4 Wire Mode [rd = 0x%02X] \r\n", ret);
...
static int bma150_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  /* raoudi-20100930 */
  i2c_smbus_write_byte_data(client, 0x14, 0x3);   // Test Code -- raoudi-20100930
//   bma150_config(client);

	/* detect and init hardware */
	if ((err = bma150_detect(client, NULL))) {
    printk("No Detect BMA150 Sensor... \n");
		goto error_1;
	}
#if 0
...
#else   // raoudi-20100928
#if 1
  printk("########################################## \n");
  printk("%s : %s found\n", __FUNCTION__, id->name);
  printk("%s : al_version=%d, ml_version=%d\n", __FUNCTION__, 
      bma150_read_bits(bma150, BMA150_AL_VERSION),
      bma150_read_bits(bma150, BMA150_ML_VERSION));
  printk("########################################## \n");
#endif    // DEUBG
