...
//
// 
//  HBE-SM3-SV210 Board : BMA150 Sensor Interface driver
//  2010.10.26
// 
//
//[*]--------------------------------------------------------------------------------------------------[*]
...
static	void bma150_sensor_read_data(void)
{
	...
	// changed X, Y 
//JNJ	temp_x = bma150_sensor_data_calc(bma150_sensor.bytes[ACC_Y_MSB], bma150_sensor.bytes[ACC_Y_LSB]);
//JNJ	temp_y = bma150_sensor_data_calc(bma150_sensor.bytes[ACC_X_MSB], bma150_sensor.bytes[ACC_X_LSB]);
//JNJ	temp_y *= (-1);
	temp_x = bma150_sensor_data_calc(bma150_sensor.bytes[ACC_X_MSB], bma150_sensor.bytes[ACC_X_LSB]);
	temp_x *= (-1);
	temp_y = bma150_sensor_data_calc(bma150_sensor.bytes[ACC_Y_MSB], bma150_sensor.bytes[ACC_Y_LSB]);
	temp_y *= (-1);
...
static int __devinit    bma150_sensor_probe	(struct platform_device *pdev)
{
  ...
  if(input_register_device(bma150_sensor.driver))	{
	...
	printk("HBE-SM3-SV210 BMA150 Sensor driver initialized!! Ver 1.0\n");
