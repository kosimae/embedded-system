...
#if defined(CONFIG_MACH_SM3SV210) || defined(CONFIG_MACH_EMPOS3)  // raoudi-20101110
#define S3C_TS_ADJUST
#endif
...
#ifdef S3C_TS_ADJUST
#define SAVED_POINT_MAX   20  // 16
#define SAVED_POINT_MOVE  5   // 3
#define MOVE_DETECT_PTR   12
#define JUMP_DETECT_PTR   3
#define RESOLUTION_X    60
#define RESOLUTION_Y    45
#define DUMP_S3CTS      3

static int s3cts_cnt = 0;

/*
 * Adjust touch values obtained SMDKC100 Board using a heuristic method.
 * Conceptually, this is a kind of low pass filer.
 */
static void smdkc100_adjust_value(struct s3c_ts_info *ts)
{
  static int history_x[SAVED_POINT_MAX], history_y[SAVED_POINT_MAX];
  static int point_move=0, point_jump=0;
  int tmp_x, tmp_y;
  int res_x, res_y;
  int saved_point;
  int valid_sample;
  ...
  saved_point = ((point_move == 0) ? SAVED_POINT_MAX : SAVED_POINT_MOVE);

  for (i = saved_point - 1; i >= 1; i--) {
  ...
  for (i = 1; i < saved_point; i++) {
    if ((ts->xp - res_x < history_x[i]) && (history_x[i] < ts->xp + res_x) &&
        (ts->yp - res_y < history_y[i]) && (history_y[i] < ts->yp + res_y)) {
      tmp_x += history_x[i];
      tmp_y += history_y[i];
      valid_sample++;
    }
  ...
  if (point_move) {
    if (valid_sample == SAVED_POINT_MOVE) {
      point_move = 0;
      for (i=SAVED_POINT_MOVE; i<SAVED_POINT_MAX; i++) {
        history_x[i] = tmp_x;
        history_y[i] = tmp_y;
      }
    }
  } else if (valid_sample < JUMP_DETECT_PTR) {
      point_move = 0;
    if (++point_jump >= 4) {
      /* Real movement of contact point */
      for (i=0; i<SAVED_POINT_MAX; i++) {
        history_x[i] = tmp_x;
        history_y[i] = tmp_y;
      }
    } else {
      /* Assume that this happed from H/W noise */
      tmp_x = history_x[1];
      tmp_y = history_y[1];
    }
  } else {
    point_jump = 0;
    if (valid_sample < MOVE_DETECT_PTR) {
      /* Drawing */
      point_move = 1;
    }
  }
  ts->xp = history_x[0] = tmp_x;
  ts->yp = history_y[0] = tmp_y;
}
#endif
...
static void touch_timer_fire(unsigned long data)
{
  ...
  if (updown) {
    //    printk(" s3cts_cnt = [%d]\t", s3cts_cnt);
    if (ts->count) {
#ifdef CONFIG_TOUCHSCREEN_S3C_DEBUG
      {
        struct timeval tv;
        do_gettimeofday(&tv);
       	printk(KERN_INFO "TS.T: %06d, X: %03ld, Y: %03ld\t", (int)tv.tv_usec, ts->xp, ts->yp);
      }
#endif

#ifdef S3C_TS_ADJUST
      smdkc100_adjust_value(ts);

      if(s3cts_cnt++ > DUMP_S3CTS) {
#ifdef CONFIG_TOUCHSCREEN_S3C_DEBUG
      {
        printk("Adjust Value-> X: %03ld, Y: %03ld , CNT: %d\n", ts->xp, ts->yp, s3cts_cnt);
      }
#endif
#endif    // S3C_TS_ADJUST
        ...
        x=(int) ts->yp/ts->count;
        y=(int) ts->xp/ts->count;
#else
...
#ifdef S3C_TS_ADJUST
  if(!updown) {
    if(s3cts_cnt > DUMP_S3CTS ) s3cts_cnt = 0;
  }
#endif
#ifdef CONFIG_TOUCHSCREEN_S3C_DEBUG
       printk(KERN_INFO "   %c\n",	updown ? 'D' : 'U');
