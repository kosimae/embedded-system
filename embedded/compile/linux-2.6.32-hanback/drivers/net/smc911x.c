...
#if defined(CONFIG_MACH_EMPOS3) || defined(CONFIG_ADKSV210) || defined(CONFIG_MACH_EDKSV210) || defined(CONFIG_MACH_SM3SV210)
...
static get_ethernet_addr(char *ethernet_addr)
{
  char *ethaddr_str;
  char temp_ethaddr[17];
  char *argptr;

  argptr = boot_command_line;
  /* Check command line */
  ethaddr_str = strstr(argptr, "ethaddr=");
  if (!ethaddr_str)
    return -1;
  ethaddr_str += strlen("ethaddr=");
  strncpy(temp_ethaddr, ethaddr_str, 17);
  str2eaddr(ethernet_addr, ethaddr_str);

#if 0
  int i;
  printk("get_ethernet_addr()---->start\n");
  for (i = 0; i < 5; i++)
    printk("%2.2x:", ethernet_addr[i]);
  printk("%2.2x\n", ethernet_addr[5]);
  printk("get_ethernet_addr()---->end\n");
#endif
  return 0;
}
#endif
