...
#undef HDMI_HIGH   // raoudi-20100903
//#define HDMI_HIGH   // raoudi-20100903
...
{
#ifdef HDMI_HIGH    // raoudi-20101025
	if (gpio_get_value(S5PV210_GPH1(5))) {
#else
	if (!gpio_get_value(S5PV210_GPH1(5))) {
#endif
#if 0   // raoudi-20101025
...
#if 1   // raoudi-20101025
#ifdef HDMI_HIGH
  if (atomic_read(&hpd_struct.state))
#else
  if (!atomic_read(&hpd_struct.state))    // raoudi-20100903
#endif
...
int irq_hdmi(int irq)
{
	...
	s5p_hdmi_disable_interrupts(HDMI_IRQ_HPD_UNPLUG);
	s5p_hdmi_disable_interrupts(HDMI_IRQ_HPD_PLUG);
#else     // raoudi-20101025
#ifdef HDMI_HIGH
  if (s5p_hdmi_get_hpd_status())
#else
  if (!s5p_hdmi_get_hpd_status())   // raoudi-20100903
#endif
    s5p_hdmi_disable_interrupts(HDMI_IRQ_HPD_UNPLUG);
  else
    s5p_hdmi_disable_interrupts(HDMI_IRQ_HPD_PLUG);
