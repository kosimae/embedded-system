...
#define BACKLIGHT_GPIO  S5PV210_GPG1(4)
#define VDD5_GPIO   S5PV210_GPH3(5)
#endif
int s3cfb_backlight_onoff(struct platform_device *pdev, int onoff)
{
	int err;
#if defined(CONFIG_MACH_EMPOS3) || defined(CONFIG_MACH_ADKSV210) || defined(CONFIG_MACH_EDKSV210) || defined(CONFIG_MACH_SM3SV210)
  printk("\t#SDKIM %s onoff:%d\n", __func__, onoff);
  /* SDKIM Enable VDD_5V */
  err = gpio_request(VDD5_GPIO, "VDD5V");
  if(err){
    printk("fail to request GPIO:%d for VDD5V\n", VDD5_GPIO);
  }
  gpio_direction_output(VDD5_GPIO, 1);

  gpio_free(VDD5_GPIO);

  /* OnOff LCD_5.0V */
  err = gpio_request(BACKLIGHT_GPIO, "BACKLIGHT");
