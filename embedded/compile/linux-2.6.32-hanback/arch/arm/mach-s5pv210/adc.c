...
unsigned int s3c_adc_convert(void)
{
  ...
  //printk("%s[%d] adc_port=%d, (adc_port & 0x7)=%d\n", __func__, __LINE__, adc_port, (adc_port & 0x7));
...
int s3c_adc_get_adc_data(int channel)
{
  ...
  //printk("%s[%d] channel=%d, adc_port=%d\n", __func__, __LINE__, channel, adc_port);
  cur_adc_port = adc_port;
  adc_port = channel;
  //printk("%s[%d] cur_adc_port=%d, adc_port=%d\n", __func__, __LINE__, cur_adc_port, adc_port);

  if(adc_clk_enable){
    adc_value = s3c_adc_convert();
  }
  else{
    printk(" adc clk enable NO\n");
  }
...
#if 1 // by netmania
  /* for adc read.. */
  writel(0x58, base_addr + S3C_ADCTSC);
#endif
