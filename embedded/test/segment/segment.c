...
//Global variable
static unsigned int segment_usage = 0; 		// segment가 사용중임을 나타냄
static unsigned long *segment_data;		// 가상주소 매핑
...
ssize_t segment_write(struct file *inode, const char *gdata, size_t length,loff_t *off_what){
	// data에는 segment에 출력되는 숫자
	// digit은 segment의 위치를 나타낸다.
	...
	if(num!=0){ // num 이 0 이 아닐때 
		...
		// 시간을 나타낼 때 2, 4번째 LED의 하단부에 점이 켜진다.
...
static int segment_ioctl(struct inode *inode, struct file *flip, unsigned int cmd, unsigned long arg){
	...
	case MODE_1_CLOCK_FORM:
		...
		break;
	default:
		return -EINVAL;	// Invalid argument -- 잘못된 인자
	}
